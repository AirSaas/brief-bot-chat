...
export default function DownloadPDFModal({ isOpen, onClose }: DownloadPDFModalProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const generatePDF = async () => {
        ...
        doc.text('Project Plan - AirSaas AI Assistant', margin, yPosition);
        ...
        if (!lastAssistantMessage || !lastAssistantMessage.content) {
          doc.setFontSize(12);
          doc.text('No content generated yet.', margin, yPosition);
        } else {
        ...
        for (let i = 1; i <= pageCount; i++) {
          ...
          doc.text('AirSaas AI Assistant', margin, doc.internal.pageSize.getHeight() - 10);
        }
  ...
  const handleShareLinkedIn = () => {
    const shareText = encodeURIComponent("I just used this amazing free tool to create my project plan! Check it out:");
    ...
    <div className="fixed inset-0 bg-gray-50/75 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4 relative">
        ...
        <p className="text-sm text-black mb-6 text-center">
          Cet outil est gratuit. Si cela vous a aidé, partagez-le sur LinkedIn!
        </p>
